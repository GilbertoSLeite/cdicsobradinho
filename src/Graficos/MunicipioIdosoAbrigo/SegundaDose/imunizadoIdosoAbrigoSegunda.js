...
export default function GrafImunizadoIdosoAbrigoSeguda() {
    ...
    function FilterMaiorZero(value) {
        if ((value.idosos_ILPI_segunda > 0) || (value.dia > 21)) {
            return value
        };
    };

    const imunizadoIdosoAbrigoSegunda = imunizadoIdosoAbrigo.map(x => x).filter(FilterMaiorZero);

    const totalCasosImunizadosIdosoAbrigo = {

        labels: imunizadoIdosoAbrigoSegunda.map(x => 'Dia - ' + x.dia),
        datasets: [
            {
                label: 'Idoso Morando em Abrigo - Segunda Dose',
                borderColor: '#f06400',
                ...
                lineTension: 1,
                fill: true,
                data: imunizadoIdosoAbrigoSegunda.map(x => x.idosos_ILPI_segunda),
            },
        ],
    };
    ...
    const totalImunizadosIdosoAbrigo = {
        ...
        title: {
            display: true,
            fontColor: '#00a63d',
            text: 'Idoso Morando em Abrigo - Segunda Dose',
        },
        scales: {
            xAxes: [
                {
