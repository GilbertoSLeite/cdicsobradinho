...
export default function GrafImunizadoAgenteSaudeSegunda() {
    ...
    function FilterMaiorZero(value) {
        if (value.trabalhador_saude_segunda > 0) {
            return value
        };
    };

    const imunizadoSaudeSeguda = imunizadoSaude.map(x => x).filter(FilterMaiorZero)

    const totalCasosImunizadosSaude = {

        labels: imunizadoSaudeSeguda.map(x => 'Dia ' + x.dia),
        datasets: [
            {
                label: 'Trabalhadores de Saúde - Segunda Dose',
                ...
                lineTension: 1,
                fill: false,
                data: imunizadoSaudeSeguda.map(x => x.trabalhador_saude_segunda),
            },
        ],
    };
    ...
    const totalImunizadosSaude = {
        ...
        title: {
            display: true,
            fontColor: '#00a63d',
            text: 'Trabalhadores de Saúde - Segunda Dose',
        },
        scales: {
            xAxes: [
                {
