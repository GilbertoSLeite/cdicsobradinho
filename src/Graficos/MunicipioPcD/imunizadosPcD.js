...
export default function GrafImunizadoPcD() {
    ...
    function FilterMaiorZero(value) {
        if (value.cidadao_especial > 0) {
            return value
        };
    };

    const imunizadoPcDPrimeira = imunizadoPcD.map(x => x).filter(FilterMaiorZero)

    const totalCasosImunizadosPcD = {

        labels: imunizadoPcDPrimeira.map(x => 'Dia - ' + x.dia),
        datasets: [
            {
                label: 'PcD - Pessoa com Deficiência - Primeira Dose',
                borderColor: '#f06400',
                ...
                lineTension: 1,
                fill: true,
                data: imunizadoPcDPrimeira.map(x => x.cidadao_especial),
            },
        ],
    };
    ...
    const totalImunizadosImunizadosPcD = {
        ...
        title: {
            display: true,
            fontColor: '#00a63d',
            text: 'PcD - Pessoa com Deficiência - Primeiro Dose',
        },
        scales: {
            xAxes: [
                {
