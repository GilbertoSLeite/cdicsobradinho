...
export default function GrafImunizadoIdosoSegunda() {
    ...
    function FilterMaiorZero(value) {
        if (value.idoso_segunda > 0) {
            return value
        };
    };

    const imunizadoIdosoSegunda = imunizadoIdoso.map(x=>x).filter(FilterMaiorZero);

    const totalCasosImunizadosIdoso = {

        labels: imunizadoIdosoSegunda.map(x => 'Dia ' + x.dia),
        datasets: [
            {
                label: 'Idosos - Segunda Dose',
                ...
                lineTension: 1,
                fill: false,
                data: imunizadoIdosoSegunda.map(x => x.idoso_segunda),
            },
        ],
    };
